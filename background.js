 ...
 *   worker builds its own out of the membership already on disk — see
 *   `buildQueue`, which only lists the mailbox when there is none. That matters
 *   here rather than in the panel: this runs on **every** wake, and an idle
 *   worker is ended after ~30 seconds, so a long pass comes back through here
 *   many times over.
 */
async function measure(order) {
  ...
  try {
    ...
    trace('measure', order ? 'started on the panel’s queue' : 'started on a queue of its own', {
